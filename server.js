...
var config = require('./config.js').additionalConfig;
...
router.get('/jira/url', function(req, res){
	res.send(config.jiraUrl);
});

router.get('/jira/sprint/:id', function(req, res){
	populateBoardFromJiraSprint(req.params.id, res);
});

router.get('/scrumblr/board/list', function(req, res){
	db.getAllRooms(function(allRooms) {
		res.json(allRooms);
	});
});

router.post('/edit-card/:id', function (req, res) {
	//do nothing
	res.send(req.body.value);
});

router.post('/edit-column', function (req, res) {
	//do nothing
	res.send(req.body.value);
});
...
function populateBoardFromJiraSprint(roomId, res) {
	...
	request({
		...
		if (!error && response && response.statusCode == 200) {
			...
			console.error("Unable to connect to Jira: " + config.jiraUrl);
			if (response) {
